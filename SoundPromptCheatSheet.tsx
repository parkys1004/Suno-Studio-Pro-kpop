...
import { PROMPT_CHEAT_SHEET } from './constants';

interface SoundPromptCheatSheetProps {
    onAddTag: (tag: string) => void;
    setTooltip: (tooltip: {text: string, x: number, y: number} | null) => void;
}

const SoundPromptCheatSheet = ({ onAddTag, setTooltip }: SoundPromptCheatSheetProps) => {
    ...
    return (
        <div style={{ border: '1px solid #374151', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            {isOpen && (
                <div style={{ backgroundColor: '#111827', padding: '15px', maxHeight: '400px', overflowY: 'auto' }}>
                    <div style={{ textAlign: 'center', fontSize: '12px', color: '#9ca3af', marginBottom: '15px' }}>
                        마우스를 올리면 상세 설명(말풍선)이 나타납니다.
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '15px' }}>
                        {PROMPT_CHEAT_SHEET.map((category, idx) => (
                            <div key={idx} style={{ backgroundColor: '#1f2937', borderRadius: '8px', border: '1px solid #374151', padding: '12px' }}>
...
export default SoundPromptCheatSheet;